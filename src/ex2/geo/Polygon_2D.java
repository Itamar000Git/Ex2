...
public class Polygon_2D implements GeoShape{
	...
	public String toString() {				//done
		...
		//return this.Apoly.toString();// poly[counter].x()+","+poly[counter].y();
		return polySrt;
		////////////////////////////////
	}
	@Override
	public boolean contains(Point_2D ot) {
		...
		for (int i=0; i<Apoly.size();i++){
			if (i==Apoly.size()-1){
				...
				if (cross){counter++;}				///need to add here if the optional point is on two segments
			}
			else {
				cross=doSegmentsIntersect(ot,outsideP,Apoly.get(i),Apoly.get(i+1));
				if (cross){counter++;}				//also here
			}
	}
		if (counter % 2 == 1) {
			contain=true;
	...
	public Point_2D outSidePoint(){			//workes like magic
		...
		//outsideP= new Point_2D(Apoly.get(0));
